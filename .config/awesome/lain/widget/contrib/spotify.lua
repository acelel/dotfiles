...
local function worker(user_args)
    ...
    local dim_opacity = args.dim_opacity or 0.5
    local max_length = args.max_length or -1
    ...
    spotify_widget = wibox.widget {
        ...
        set_text = function(self, artist, song)
            ...
            if self:get_children_by_id('artistw')[1]:get_markup() ~= markup.fontfg(font, "#9651f0", '  ' .. artist_to_display) then
                self:get_children_by_id('artistw')[1]:set_markup(markup.fontfg(font, "#9651f0", '  ' .. artist_to_display))
            end
            local title_to_display = ellipsize(song, max_length)
            if self:get_children_by_id('titlew')[1]:get_markup() ~= markup.fontfg(font, "#9651f0", title_to_display .. ' ') then
                self:get_children_by_id('titlew')[1]:set_markup(markup.fontfg(font, "#9651f0", title_to_display .. ' '))
            end
        end
    }
